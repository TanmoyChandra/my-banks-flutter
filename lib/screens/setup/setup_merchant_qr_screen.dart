  ...
  void _handleSave(WalletProvider provider) {
    ...
    if (_upiIdController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Please enter a UPI ID. This is mandatory for merchants.',
          ),
        ),
      );
      return;
    ...
    final newMerchant = MerchantQR(
      ...
      qrValue: _qrValueController.text.trim().isNotEmpty
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          _showForm
              ? (_editId != null ? 'Edit Merchant QR' : 'Add Merchant QR')
              : 'Merchant QR Codes',
        ...
        leading: IconButton(
          ...
          onPressed: () => _showForm ? _resetForm() : context.pop(),
        ),
      ),
      body: _isLoading
